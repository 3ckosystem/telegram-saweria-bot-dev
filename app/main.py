...
# --- Helpers untuk resolve username & judul chat di sisi server ---
_USERNAME_RE = re.compile(r"^[A-Za-z0-9_]{5,32}$")
def _valid_usernames(items: List[str]) -> List[str]:
    return [x.strip() for x in items if _USERNAME_RE.fullmatch(x.strip())]

_NAME_CACHE_SRV: dict[str, str] = {}

async def _title_server(chat_id: str) -> str:
    """Ambil title chat, cache-kan; fallback ke @username atau id."""
    key = str(chat_id)
    if key in _NAME_CACHE_SRV:
        return _NAME_CACHE_SRV[key]
    try:
        chat = await bot_check.get_chat(chat_id=key)
        title = getattr(chat, "title", None) or f"@{getattr(chat, 'username', '')}".strip("@") or key
    except Exception:
        title = key
    if len(title) > 32:
        title = title[:29] + "..."
    _NAME_CACHE_SRV[key] = title
    return title
...
# ---------- Gate Status (dikembalikan dengan nama/title & link) ----------
@app.get("/api/gate/status")
async def gate_status(uid: int = Query(..., description="Telegram user_id")):
    # Baca ENV setiap request (biar runtime-reload)
    ...
    channel_links = _split_env("REQUIRED_CHANNEL_INVITES")
    group_users   = _valid_usernames(_split_env("REQUIRED_GROUP_USERNAMES"))
    channel_users = _valid_usernames(_split_env("REQUIRED_CHANNEL_USERNAMES"))
    ...
    for i, cid in enumerate(group_ids):
        res = await _is_member_server(uid, cid)  # 1,0,-1
        if res == 1: ok_count += 1
        if res == -1: any_cannot = True
        title = await _title_server(cid)
        items.append({
            "kind": "group",
            "id": cid,
            "title": title,
            "invite": group_links[i] if i < len(group_links) else "",
            "username": group_users[i] if i < len(group_users) else "",
            "is_member": (True if res == 1 else False if res == 0 else None),
            "can_check": (res != -1),
        })
    ...
    for i, cid in enumerate(channel_ids):
        ...
        items.append({
            ...
            "id": cid,
            "title": title,
            "invite": channel_links[i] if i < len(channel_links) else "",
            "username": channel_users[i] if i < len(channel_users) else "",
            "is_member": (True if res == 1 else False if res == 0 else None),
            "can_check": (res != -1),
        })
    ...
    # 403 + detail lengkap untuk UI WebApp
    detail = {
        "passed": False,
        "ok_count": ok_count,
        "total_required": total_required,
        "mode": mode,
        "min_count": min_count,
        "items": items,  # ← front-end render label dari sini
        # kompat lama (jika masih dipakai)
        "group_invites": group_links,
        "channel_invites": channel_links,
        "group_usernames": group_users,
        "channel_usernames": channel_users,
    }
